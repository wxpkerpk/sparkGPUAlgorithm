...
import org.apache.spark.calabar.ml.recommendation.ALSClKernel
import org.apache.spark.internal.Logging
import org.apache.spark.ml.{Estimator, Model}
import org.apache.spark.ml.param._
import org.apache.spark.ml.recommendation.ALS.{ALSPartitioner, LocalIndexEncoder}
...
/**
  ...
  */
@Since("1.3.0")
class ALSModelWithGPU private[ml](
                                   @Since("1.4.0") override val uid: String,
                                   @Since("1.4.0") val rank: Int,
                                   @transient val userFactors: DataFrame,
                                   @transient val itemFactors: DataFrame)
  extends Model[ALSModelWithGPU] with ALSModelParams with MLWritable {
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): ALSModelWithGPU = {
    val copied = new ALSModelWithGPU(uid, rank, userFactors, itemFactors)
    copyValues(copied, extra).setParent(parent)
  }

  @Since("1.6.0")
  override def write: MLWriter = new ALSModelWithGPU.ALSModelWriter(this)
...
@Since("1.6.0")
object ALSModelWithGPU extends MLReadable[ALSModelWithGPU] {

  private val NaN = "                                                                                                                                                                                                                                                            n an"
  private val Drop = "drop"
  private[recommendation] final val supportedColdStartStrategies = Array(NaN, Drop)

  @Since("1.6.0")
  override def read: MLReader[ALSModelWithGPU] = new ALSModelReader

  @Since("1.6.0")
  override def load(path: String): ALSModelWithGPU = super.load(path)

  private class ALSModelWriter(instance: ALSModelWithGPU) extends MLWriter {
  ...
  private class ALSModelReader extends MLReader[ALSModelWithGPU] {

    /** Checked against metadata when loading model */
    private val className = classOf[ALSModelWithGPU].getName

    override def load(path: String): ALSModelWithGPU = {
      ...
      val model = new ALSModelWithGPU(metadata.uid, rank, userFactors, itemFactors)
...
/**
  ...
  */
@Since("1.3.0")
class ALSWithGPU(@Since("1.4.0") override val uid: String) extends Estimator[ALSModelWithGPU] with ALSParams
  with DefaultParamsWritable {

  import org.apache.spark.ml.recommendation.ALSWithGPU.Rating
  ...
  override def fit(dataset: Dataset[_]): ALSModelWithGPU = {
    transformSchema(dataset.schema)
    ...
    val (userFactors, itemFactors) = ALSWithGPU.trainWithGPU(ratings = ratings, rank = $(rank),
    ...
    val model = new ALSModelWithGPU(uid, $(rank), userDF, itemDF).setParent(this)
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): ALSWithGPU = defaultCopy(extra)
}
...
object ALSWithGPU extends DefaultParamsReadable[ALSWithGPU] with Logging {

  import com.github.fommil.netlib.LAPACK.{getInstance => lapack}

  def solve(A: Array[Double], bx: Array[Double], lamdba: Double): Array[Double] = {
    val k = bx.length
    val trik = k * (k + 1) / 2
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    val re = solve(a, Array(8.7,
    //      -13.35,
    //      2.89,
    //      -4.14))
    val sc = new SparkContext("local", "kmeans")
    val dataArray = new Array[(Int, Int, Float)](10000)
    var uu = new Random(6)
    (0 until 10000).indices.foreach(x =>

      dataArray(x) = (math.abs(uu.nextInt() % 100000), math.abs(uu.nextInt() % 100000), math.abs(uu.nextInt() % 100f))
    )
    ...
    val dataset = data.toDF("user", "item", "rating")
    dataset.cache()


    val start1=System.currentTimeMillis()
    val model1 = new ALSWithGPU().setRank(40).setSeed(3).setNumItemBlocks(1).setNumUserBlocks(1).setMaxIter(1).fit(dataset = dataset)
    val end1=System.currentTimeMillis()
    val start2=System.currentTimeMillis()
//    val model2 = new ALS().setRank(40).setSeed(3).setNumItemBlocks(6).setNumUserBlocks(6).setMaxIter(1).fit(dataset = dataset)
//    val end2=System.currentTimeMillis()
//    val m1i = model1.userFactors.collect()
//    val m2i = model2.userFactors.collect()
//    val m1u = model1.itemFactors.collect()
//    val m2u = model2.itemFactors.collect()
    println(end1-start1)
    println(ALSClKernel.getInstance().periods)
//    println(end2-start2)
//    m1u.clone()
  ...
  @Since("1.6.0")
  override def load(path: String): ALSWithGPU = super.load(path)

  /** Trait for least squares solvers applied to the normal equation. */
  private[recommendation] trait LeastSquaresNESolver extends Serializable {
    /** Solves a least squares problem with regularization (possibly with other constraints). */
    def solve(ne: NormalEquation, lambdasolve: Double): Array[Float]
  }
  ...
  /**
    ...
    */
  def trainWithGPU[ID: ClassTag]( // scalastyle:ignore
    ...
    val blockRatings = partitionRatings(ratings = ratings, userPart, itemPart)
    ...
    if (implicitPrefs) {
      for (iter <- 1 to maxIter) {
        ...
        if (shouldCheckpoint(iter)) {
          ALSWithGPU.cleanShuffleDependencies(sc, deps)
  ...
  private def computeFactors[ID](
    ...
    //        val factorsArray = new ArrayBuffer[Float]()
    //        val ratinsArray = new ArrayBuffer[Float]()
    //        val perNumArray = new ArrayBuffer[Int]()
    //        val lambdaArray=new ArrayBuffer[Float]()
    //        var j = 0
    ...
    //          var i = srcPtrs(j)
    //          var numExplicits = 0
    //          perNumArray += srcPtrs(j + 1) - i
    //          while (i < srcPtrs(j + 1)) {
    //            val encoded = srcEncodedIndices(i)
    //            val blockId = srcEncoder.blockId(encoded)
    //            val localIndex = srcEncoder.localIndex(encoded)
    //            val srcFactor = sortedSrcFactors(blockId)(localIndex)
    //            factorsArray ++= srcFactor
    //
    ...
    //          lambdaArray+=(numExplicits * regParam).toFloat
    ////          dstFactors(j) = solver.solve(ls, numExplicits * regParam)
    //          j += 1
    //        }
    //        ALSClKernel.getInstance().run(factorsArray.toArray,ratinsArray.toArray,perNumArray.toArray,lambdaArray.toArray,rank,dstFactors)
    //        dstFactors
    //    }


    //    val numPartitions=      dstInBlocks.join(merged).getNumPartitions
    //   val rdd= dstInBlocks.join(merged).mapPartitions {                         dstInBlocks.join(merged)
    //      iter =>
    //        //这种java风格的写法在这儿性能更高,可是奇丑无比,连idea都看不下去了
    //        val srcFactorsArrays = new ArrayBuffer[Float]()
    //        val ratingArrays = new ArrayBuffer[Float]()
    //        val originalLengths = new ArrayBuffer[Int]()
    //        val lamdbaArrays = new ArrayBuffer[Float]()
    //        val perNum = new ArrayBuffer[Int]()
    //
    ...
    //          count+=1
    //          val value = x._2
    //          val dstIds = value._1.srcIds
    //          val srcPtrs = value._1.dstPtrs
    //          val srcEncodedIndices = value._1.dstEncodedIndices
    //          val ratings = value._1.ratings
    //          val srcFactors = value._2
    //          val sortedSrcFactors = new Array[FactorBlock](numSrcBlocks)
    //          srcFactors.foreach { case (srcBlockId, factors) =>
    //            sortedSrcFactors(srcBlockId) = factors
    //          }
    //          var j = 0
    //          originalLengths += dstIds.length
    //          while (j < dstIds.length) {
    //            var i = srcPtrs(j)
    //            var numExplicits = 0
    //            perNum += srcPtrs(j + 1) - i
    //            while (i < srcPtrs(j + 1)) {
    //              val encoded = srcEncodedIndices(i)
    //              val blockId = srcEncoder.blockId(encoded)
    //              val localIndex = srcEncoder.localIndex(encoded)
    //              val srcFactor = sortedSrcFactors(blockId)(localIndex)
    //              val rating = ratings(i)
    //              srcFactorsArrays ++= srcFactor
    //              ratingArrays += rating
    //              numExplicits += 1
    //              i += 1
    //            }
    //            lamdbaArrays += (numExplicits * regParam).toFloat
    //            j += 1
    //          }
    //        }
    //
    //        if(srcFactorsArrays.nonEmpty) {
    //          println("++++++++++++")
    //          val resultArray=new Array[Array[Array[Float]]](count)
    //          val result = ALSClKernel.getInstance().run(srcFactorsArrays.toArray,
    //            ratingArrays.toArray, originalLengths.toArray
    //            , perNum.toArray, lamdbaArrays.toArray, rank,resultArray)//gpu运行求解
    //          dataArray.indices.map(x => {
    //            (dataArray(x)._1, result(x))
    //          }).foreach(println(_))
    //          dataArray.indices.map(x => {
    //            (dataArray(x)._1, result(x))
    //          }).iterator
    ...
    val time1 = dstInBlocks.context.longAccumulator
    val time2 = dstInBlocks.context.longAccumulator

    val result= dstInBlocks.join(merged).mapValues {
      case (InBlock(dstIds, srcPtrs, srcEncodedIndices, ratings), srcFactors) =>
        val sortedSrcFactors = new Array[FactorBlock](numSrcBlocks)
        val start1=System.currentTimeMillis()
        srcFactors.foreach { case (srcBlockId, factors) =>
