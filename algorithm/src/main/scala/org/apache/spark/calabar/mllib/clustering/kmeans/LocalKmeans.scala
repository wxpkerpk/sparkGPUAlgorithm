...
object LocalKMeans extends Logging {
  ...
  def kMeansPlusPlus(
    ...
    val costArray = points.map(KMeansWithGPU.fastSquaredDistance(_, centers(0)))
    ...
    for (i <- 1 until k) {
      ...
      for (p <- points.indices) {
        costArray(p) = math.min(KMeansWithGPU.fastSquaredDistance(points(p), centers(i)), costArray(p))
      }
    ...
    while (moved && iteration < maxIterations) {
      ...
      while (i < points.length) {
        val p = points(i)
        val index = KMeansWithGPU.findClosest(centers, p)._1
    ...
    if (iteration == maxIterations) {
      logInfo(s"Local KMeans.cl++ reached the max number of iterations: $maxIterations.")
    } else {
      logInfo(s"Local KMeans.cl++ converged in $iteration iterations.")
    }
